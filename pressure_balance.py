...
#    PRESSURE_THRESHOLD_N=57.7N은 최종 실험 기준 검증이 남은 잠정값이다.
...
# PRESSURE_THRESHOLD_N은 구동부 기준 및 수조 실험 데이터로 검증이 필요하다.
# 정연 본인이 "메인" 페이지의
# "남은 실제 연결 작업" 체크리스트에 "수조 실험 후 LOW/MID/HIGH
# 기준값 및 57.7N 임계값 보정"이라고 직접 명시해뒀다 (2026-08-25).
# 확정한 폭을 사용하되, 임계값과 개방 동작은 수조 실험으로 검증해야 한다.
WINDOW_WIDTH_M = 0.37        # 포맥스판 수압 작용 면 가로 길이 37cm (확정)
PRESSURE_THRESHOLD_N = 57.7  # TODO(잠정치): 수조 실험 및 구동부 기준 검증 필요
